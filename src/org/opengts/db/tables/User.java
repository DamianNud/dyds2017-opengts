    ...
    implements UserInformation
{
    ...
    **/
    public void updatePasswordFields()
        throws DBException
    {
        this.update(User.FLD_password, User.FLD_tempPassword);
    }
    ...
    **/
    public void suspendOnLoginFailureAttempt(boolean addCurrentFailure)
    {
        // -- suspend user if too many failed logins
        String accountID = this.getAccountID();
        String userID    = this.getUserID();
        int    addCount  = addCurrentFailure? 1 : 0;
        long failedInterval = RTConfig.getLong(DBConfig.PROP_User_failedLoginInterval,DateTime.MinuteSeconds(3));
        int  failedAttempts = RTConfig.getInt( DBConfig.PROP_User_failedLoginAttempts,6);
        if ((failedInterval > 0L) && (failedAttempts > 0)) {
            long nowTime   = DateTime.getCurrentTimeSec();
            long sinceTime = nowTime - failedInterval;
            int count = Audit.getFailedLoginAttampts(accountID, userID, sinceTime) + addCount;
        ...
        // -- check account as well
        Account acct = this.getAccount(); // should never be null
        if (acct != null) {
            acct.suspendOnLoginFailureAttempt(addCurrentFailure);
        }
    }
    ...
    */
        ...
        throws DBException
    {
        ...
        if (RTConfig.getBoolean(DBConfig.PROP_User_authorizedPreferredDeviceID,false)) {
