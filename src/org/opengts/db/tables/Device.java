    ...
    implements DataTransport
{
    ...
    **/
    ...
    public static void initELogHOSProvider()
    {
        if (Device.elogHosProvider == null) {
            String elogClassName = DBConfig.PACKAGE_ATSELOG + ".ClientQueue";
            try {
                Class<?> elogClass = (Class<ELogHOSProvider>)Class.forName(elogClassName);
                Device.elogHosProvider = (ELogHOSProvider)elogClass.newInstance();
                String elogVers = Device.elogHosProvider.getVersion();
                String elogDesc = Device.elogHosProvider.getDescription();
                Print.logDebug("Installed ELogHOSProvider: ["+StringTools.className(Device.elogHosProvider)+"] v"+elogVers+" "+elogDesc);
            } catch (ClassNotFoundException cnfe) {
                // -- ELogHOSProvider class not found
            } catch (ClassCastException cce) {
                // -- specified class is not a ELogHOSProvider
                Print.logError("Invalid ELogHOSProvider class: " + elogClassName);
            } catch (Throwable th) { // ClassNotFoundException, ClassCastException
    ...
    **/
    public static void setELogHOSProvider(ELogHOSProvider elp)
    {
        if (elp != null) {
            Device.elogHosProvider = elp;
            Print.logDebug("Device ELogHOSProvider installed: " + StringTools.className(Device.elogHosProvider));
        } else
        if (Device.elogHosProvider != null) {
            Device.elogHosProvider = null;
            Print.logDebug("Device ELogHOSProvider removed.");
        }
    }
    ...
    **/
    ...
    {
        return (Device.elogHosProvider != null);
    }
  ...
  //public static final String FLD_maintDescriptionHR0   = "maintDescriptionHR0";   // hours maint #0 description
    ...
    protected boolean _insertEventData(final EventData evdb)
    {
        ...
        */
        ...
        if (Device.GetSimulateEngineHours(this)) {
            // -- force to recalculate: if (...) { evdb.setEngineHours(0.0); }
            if (evdb.getEngineHours() > 0.0) {
                // -- event already has engine hours, leave as-is
                Print.logInfo("["+acctID+"/"+devID+"] SimEngHours: Event engine-hours already set (leaving as-is): " + evdb.getEngineHours());
            } else
    ...
    **/
    private void _postDeviceEventUpdate()
    {
        ...
        /* ELog/HOS */
        if (Device.hasELogHOSProvider() && Device.IsELogEnabled(this)) {
            // -- add ELog/HOS job to thread
            ELogHOSProvider elp = Device.getELogHOSProvider();
        ...
        /* post event to custom handler */
        //if (this.hasCustomEventHandler() && this.supportsDataPushTime()) {
        //    // -- post received events for device
        //    CustomEventHandler ceh = this.getCustomEventHandler();
        //    int stat = ceh.processDeviceEvents(this);
        //    if (stat >= CustomEventHandler.ERR_ERROR) {
        //        Print.logWarn("CustomEventHandler status error: " + stat);
        //    }
        //}
    ...
    *** Note: <br>
    ***  - "evdb.getDevice()" may be a copy of "this" Device instance, but
    *** made prior to any adjustments made from the EventData instance.<br>
    ***  - This event likely does not yet have an address assigned yet by the
    *** Reverse-Geocoder.
    **/
        ...
        Locale locale, BasicPrivateLabel bpl, Device dev)
    {
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_VOLUME)) {
            if (getTitle) {
                ...
                Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_1;
                double L = dev.getFuelCapacity(tank) * dev.getLastFuelLevel(tank); // tank #1
        ...
        if (EventData._keyMatch(key,Device.KEY_FUEL_VOLUME2)) {
            if (getTitle) {
                ...
                Account.VolumeUnits vu = Account.getVolumeUnits(dev.getAccount());
                Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_2;
                double L = dev.getFuelCapacity(tank) * dev.getLastFuelLevel(tank); // tank #2
                double V = vu.convertFromLiters(L);
                return StringTools.format(V,"0.0") + " " + vu.toString(locale);
            }
        } else
