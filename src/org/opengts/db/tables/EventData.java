    ...
    **/
        ...
        throws DBException
    {
        ...
        DBSelect<EventData> dsel = EventData._createRangeEventSelector(
            ...
            addtnlSelect_1, null/*addtnlSelect_1*/);
    ...
    **/
        ...
        Locale locale, BasicPrivateLabel bpl, EventData ed)
    {
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL)) {
            if (getTitle) {
                ...
                double level = ed.getFuelLevel(tank,true/*estimate*/);
                if (level < 0.0) {
                    return i18n.getString("EventData.notAvailable", "n/a");
                } else {
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_VOLUME)) {
            if (getTitle) {
                ...
                if (dev == null) {
                    ...
                    double L = dev.getFuelCapacity(tank) * ed.getFuelLevel(tank,true/*estimate*/); // tank #1
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_LEVEL2)) {
            if (getTitle) {
                ...
                double level = ed.getFuelLevel(tank,true/*estimate*/); // estimate not support for tank #2
                if (level < 0.0) {
                    return i18n.getString("EventData.notAvailable", "n/a");
                } else {
                    long pct = Math.round(level * 100.0);
        ...
        if (EventData._keyMatch(key,EventData.KEY_FUEL_VOLUME2)) {
            if (getTitle) {
                ...
                if (dev == null) {
                    ...
                    Device.FuelTankIndex tank = Device.FuelTankIndex.TANK_2;
                    Account.VolumeUnits vu = Account.getVolumeUnits(ed.getAccount());
                    double L = dev.getFuelCapacity(tank) * ed.getFuelLevel(tank,true/*estimate*/); // tank #2
                    double V = vu.convertFromLiters(L);
                    return StringTools.format(V,"0.0") + " " + vu.toString(locale);
                }
            }
    ...
    public static void main(String argv[])
    {
        ...
        if (RTConfig.getBoolean(ARG_MAKE_INNODB,false)) {
            ...
            long estSEC_lo = (long)(((double)recordCount * 0.05) / 1000.0); // 0.044
            long estSEC_hi = (long)(((double)recordCount * 0.09) / 1000.0);
