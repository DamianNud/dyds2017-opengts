    ...
    public static class SmtpProperties
    {
        ...
        public String getHost() {
            ...
            if (StringTools.isBlank(V)/* || StringTools.endsWithIgnoreCase(V,EXAMPLE_DOT_COM)*/) { 
                V = StringTools.trim(RTConfig.getString(K)); 
            }
        ...
        // ----
        public void setMultipartType(String V) { // [2.6.2-B52]
            String K = RTKey.SMTP_MULTIPART_TYPE;
            this.smtpProps.setString(K, StringTools.trim(V));
        }
        public String getMultipartType() { // [2.6.2-B52]
            String K = RTKey.SMTP_MULTIPART_TYPE;
            String V = this.smtpProps.hasProperty(K)? 
                this.smtpProps.getString(K,null) : 
                null;
        ...
        public boolean equals(Object other) {
            ...
            SmtpProperties sp = (SmtpProperties)other;
            // -- Debug
            if (this.getDebug() != sp.getDebug()) {
                //Print.logInfo("SmtpProperties.equals: SMTP Debug differs");
                return false;
            }
            // -- Host
            if (!this.getHost().equals(sp.getHost())) {
                //Print.logInfo("SmtpProperties.equals: SMTP Host differs");
                return false;
            }
            // -- Port
            if (this.getPort() != sp.getPort()) {
                //Print.logInfo("SmtpProperties.equals: SMTP Port differs");
                return false;
            }
            // -- User name
            if (!this.getUser().equals(sp.getUser())) {
                //Print.logInfo("SmtpProperties.equals: SMTP User differs");
                return false;
            }
            // -- User email
            if (!this.getUserEmail().equals(sp.getUserEmail())) {
                //Print.logInfo("SmtpProperties.equals: SMTP UserEmail differs");
                return false;
            }
            // -- User password
            if (!this.getPassword().equals(sp.getPassword())) {
                //Print.logInfo("SmtpProperties.equals: SMTP UserPassword differs");
                return false;
            }
            // -- SSL
            if (!this.getEnableSSL().equalsIgnoreCase(sp.getEnableSSL())) {
                //Print.logInfo("SmtpProperties.equals: SMTP SSL differs");
                return false;
            }
            // -- TLS
            if (!this.getEnableTLS().equalsIgnoreCase(sp.getEnableTLS())) {
                //Print.logInfo("SmtpProperties.equals: SMTP TLS differs");
                return false;
            }
            // -- Timeout
            if (this.getTimeoutMS() != sp.getTimeoutMS()) {
                //Print.logInfo("SmtpProperties.equals: SMTP Timeout differs");
                return false;
            }
            // -- RetryCount
            if (this.getRetryCount() != sp.getRetryCount()) {
                //Print.logInfo("SmtpProperties.equals: SMTP RetryCount differs");
                return false;
            }
            // -- Sysadmin email
            if (!this.getSysadminEmail().equals(sp.getSysadminEmail())) {
                //Print.logInfo("SmtpProperties.equals: SMTP SysadminEmail differs");
                return false;
            }
            // -- equals
            return true;
