        ...
        if (updateAccount) {
            if (selAccount == null) {
                ...
                try {
                    ...
                    // -- showEnableDriverELogs
                    if (showEnableDriverELogs && isSysAdmin) {
                        boolean elogEnable = ComboOption.parseYesNoText(locale, elogEnableStr, false);
                        if (selAccount.getELogEnabled() != elogEnable) { 
                            selAccount.setELogEnabled(elogEnable); 
                        }
                        if (!selAccount.getELogAccountID().equals(elogAccount)) {
                            // -- PARM_ACCT_ELOG_ACCOUNT : ELog AccountID/PartnerID
                            selAccount.setELogAccountID(elogAccount);
                        }
                        if (!selAccount.getELogUsername().equals(elogUser)) {
                            // -- PARM_ACCT_ELOG_USER    : ELog Username/LoginID
                            selAccount.setELogUsername(elogUser);
                        }
                        if (!selAccount.getELogPassword().equals(elogPassword)) {
                            // -- PARM_ACCT_ELOG_PASSWORD: ELog Password
                            selAccount.setELogPassword(elogPassword);
                        }
                    }
                    // -- private label name
                    if (isSysAdmin) {
              ...
              //String menuURL    = EncodeMakeURL(reqState,RequestProperties.TRACK_BASE_URI(),PAGE_MENU_TOP);
                ...
                if (_listAccounts) {
                    ...
                    if (showEnableDriverELogs && isSysAdmin) {
                        // -- "Enable Driver ELogs":
                        out.println(FormRow_Separator());
                        boolean elogEnabled  = (_selAccount != null)? _selAccount.getELogEnabled() : false;
                        ...
                        String      elogAcct = (_selAccount != null)? _selAccount.getELogAccountID() : "";
                        String      elogUser = (_selAccount != null)? _selAccount.getELogUsername() : "";
                        String      elogPass = (_selAccount != null)? _selAccount.getELogPassword() : "";
                        out.println(FormRow_ComboBox (PARM_ACCT_ELOG_ENABLE  , editSysAdmin, i18n.getString("SysAdminAccounts.enableDriverELogs","Enable Driver ELogs")+":", elogOpt, elogMap, "", -1, elogDesc));
                        out.println(FormRow_TextField(PARM_ACCT_ELOG_ACCOUNT , editSysAdmin, i18n.getString("SysAdminAccounts.elogAccountID"    ,"ELog AccountID")+":"     , elogAcct, 24, 24));
                        out.println(FormRow_TextField(PARM_ACCT_ELOG_USER    , editSysAdmin, i18n.getString("SysAdminAccounts.elogUsername"     ,"ELog Username")+":"      , elogUser, 24, 24));
                        out.println(FormRow_TextField(PARM_ACCT_ELOG_PASSWORD, editSysAdmin, i18n.getString("SysAdminAccounts.elogPassword"     ,"ELog Password")+":"      , elogPass, 24, 24));
                    }
