        ...
        if ((gbf == null) || (gbf.length == 0)) {
            ...
            if (fact != null) {
                for (int i = 0; i < gbf.length; i++) {
                    if (gbf[i] == null) {
                        Print.logError("DBFactory field is null: %s.%s", fact.getUntranslatedTableName(), gbf[i]);
                    } else
                    if (fact.hasField(gbf[i])) {
                        // -- ok as-is
                    } else {
                        Print.logError("DBFactory field does not exist: %s.%s", fact.getUntranslatedTableName(), gbf[i]);
                    }
                }
            }
