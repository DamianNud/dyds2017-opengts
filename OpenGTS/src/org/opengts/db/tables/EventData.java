    ...
    implements EventDataProvider, GeoPointProvider
{
    ...
    **/
    ...
    private static final DBField StandardFieldInfo[] = {
        ...
        // -- Common fields
        newField_creationTime(RTConfig.getBoolean(DBConfig.PROP_EventData_keyedCreationTime,false)), // FLD_creationTime
    };
  ...
  //public static final String FLD_tripMovingHours      = "tripMovingHours";
    ...
    public static final DBField CreationTimeMillisecond[] = {
        newField_creationMillis(ALTKEY_eq_pushkey), // FLD_creationMillis
  ...
  //public static final String FLD_appliedPressure      = "appliedPressure";        // kPa
    ...
    **/
    ...
    {
        Integer v = (Integer)this.getFieldValue(FLD_ignitionState);
    ...
    **/
    public void setTireState(TireState ts[])
    {
        ...
        long tireMask = 0L;
        for (int i = 0; (i < ts.length) && (i < TireState.GetMaximumTires()); i++) {
            TireState T = ts[i];
            ...
            if ((tireMask & (1L << ndx)) != 0L) {
                // -- already handled this tire index
                continue;
            }
            tireMask |= (1L << ndx);
            // -- key
            String key = T.toKeyString(false,ndx);
            // -- pressure/temperature
            boolean hasPress = T.hasActualPressure();
            boolean hasTemp  = T.hasActualTemperature();
            if (hasPress || hasTemp) {
                // -- pressure
                if (hasPress) {
                    if (sbP.length() > 0) { sbP.append(" "); }
                    sbP.append(key).append("=");
                    T.getPressureString(sbP);
                }
                // -- temperature
                if (hasTemp) {
                    if (sbT.length() > 0) { sbT.append(" "); }
                    sbT.append(key).append("=");
                    T.getTemperatureString(sbT);
                }
                // -- save in cache
                map.put(new Integer(ndx), T);
            }
        }
    ...
    **/
    public OrderedMap<Integer,TireState> getTireState()
    {
        if (this.tireStateCache == null) {
            ...
            // -- pressure
            String Ps = this.getTirePressure();
            TireState.ParseTireState(Ps,false,tsMap); // preference to pressure
            // -- temperature
            String Ts = this.getTireTemp();
            TireState.ParseTireState(Ts,true ,tsMap); // preference to temperature
            // -- cached map
  ...
  //private static final String ARG_UPDATE_ADDRESS[]    = new String[] { "updateAddress"                };
    ...
    public static void main(String argv[])
    {
        ...
        if (RTConfig.getBoolean(ARG_ADDR_LEN,false)) {
            String addrSizeKey = RTKey.DB_TYPESIZE_ + FLD_address; // "db.typeSize.address";
        ...
        if (RTConfig.getBoolean(ARG_MAKE_INNODB,false)) {
            ...
            if (!confirm) {
                ...
                "To indicate that you are aware of the time it will take to exectue this command, please add \n" +
            ...
            DBConnection dbc = null;
            try {
                String edTableName = EventData.TABLE_NAME();
